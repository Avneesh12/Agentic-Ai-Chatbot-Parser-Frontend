    ...
    <div className="flex min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      {showPanel && (
        <>
          ...
          />
          <aside
            className={`fixed left-0 top-0 h-full z-50 flex flex-col ${panelClose ? "panel-close" : "panel-open"}`}
            style={{
      ...
      {/* ── Main content ────────────────────────────────────────────────── */}
      <main className="relative z-10 flex flex-col min-h-screen w-full px-6 py-8">
            ...
            {user ? (
              <>
                ...
                <button onClick={onLogout}
                  ...
                  style={{ width: 32, height: 32, background: "var(--surface)", border: "1px solid var(--border2)", color: "var(--ink3)" }}
                  title="Sign out">
                  <LogOut size={13} />
                </button>
              </>
              ...
              <button onClick={() => setAuthOpen(true)}
                ...
                style={{ background: "var(--teal)", color: "#000", border: "none", fontFamily: "var(--font-display)" }}>
                Sign In
              </button>
            )}
          </div>
        ...
        {/* Hero */}
        <div className="flex-1 flex flex-col items-center justify-center text-center max-w-3xl mx-auto w-full py-16">

          {/* Status pill */}
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs mb-8 fade-up"
            ...
            RAG · Agentic Tools · Real-time Streaming
          </div>

          <h1 className="fade-up" style={{
            fontFamily: "var(--font-display)", fontWeight: 700,
            fontSize: "clamp(36px, 6vw, 64px)", lineHeight: 1.08,
            letterSpacing: "-1.5px", color: "var(--ink)", marginBottom: 20,
          ...
          <p className="fade-up" style={{
            fontSize: "clamp(14px, 2vw, 17px)", color: "var(--ink3)", lineHeight: 1.7,
            maxWidth: 520, marginBottom: 36, animationDelay: "0.1s",
          }}>
            {user
          ...
          {/* CTA buttons */}
          <div className="flex flex-wrap items-center justify-center gap-3 fade-up" style={{ animationDelay: "0.15s" }}>
            ...
            {user && ENABLE_RAG_UPLOAD && (
              <button onClick={() => { if (!user) { setAuthOpen(true); return; } switchView("upload"); }}
          ...
          {/* Feature grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-16 w-full max-w-2xl fade-up" style={{ animationDelay: "0.2s" }}>
            {[
              ...
              <div key={f.title} className="flex flex-col gap-2 p-4 rounded-xl text-left"
                style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                <div style={{ color: "var(--teal)" }}>{f.icon}</div>
                <p style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 13, color: "var(--ink)" }}>{f.title}</p>
                <p style={{ fontSize: 11, color: "var(--ink3)" }}>{f.desc}</p>
              </div>
            ))}
          </div>
          ...
          {/* Env flag notice */}
          {!ENABLE_RAG_UPLOAD && (
            <div className="mt-8 px-4 py-2.5 rounded-lg flex items-center gap-2 fade-up"
              style={{ background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.2)", animationDelay: "0.25s" }}>
              <Database size={12} style={{ color: "var(--amber)", flexShrink: 0 }} />
              <p style={{ fontSize: 11, color: "var(--amber)" }}>
                Document upload is disabled. Set <code style={{ background: "rgba(245,158,11,0.12)", padding: "1px 5px", borderRadius: 4 }}>NEXT_PUBLIC_ENABLE_RAG_UPLOAD=true</code> to enable.
              </p>
            </div>
          )}
        </div>
        ...
        {/* Footer */}
        <footer className="text-center" style={{ fontSize: 11, color: "var(--ink3)" }}>
