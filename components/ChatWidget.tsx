...
import {
  ...
  ChevronRight, Loader2, Clock, MessageSquare, BookOpen,
} from "lucide-react";
import KnowledgeUpload from "@/components/KnowledgeUpload";

const RAG_UPLOAD_ENABLED = process.env.NEXT_PUBLIC_ENABLE_RAG_UPLOAD === "true";
...
type Tab = "chat" | "history" | "knowledge";
...
function StreamingText({ text, isStreaming }: { text: string; isStreaming?: boolean }) {
  ...
  if (!isStreaming) return <span style={{ whiteSpace: "pre-wrap" }}>{text}</span>;

  return (
    <span style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
      {displayedChars.map((char, i) =>
        i >= animatedUpTo ? (
          <span key={i} className="streaming-char"
            style={{ animationDelay: `${(i - animatedUpTo) * 0.025}s` }}>{char}</span>
        ) : (
...
function MessageBubble({ msg }: { msg: LocalMessage }) {
  ...
  return (
    <div className={`flex items-end gap-3 msg-enter ${isUser ? "flex-row-reverse" : ""}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
        isUser ? "bg-gradient-to-br from-pink-500 to-purple-600"
        ...
        <div className="flex items-center gap-2 px-1">
          ...
          {msg.source && !msg.isStreaming && (
            <span className={`text-xs px-2 py-0.5 rounded-full border ${
              msg.source.startsWith("TOOL") ? "source-tool"
              : msg.source === "RAG"        ? "source-rag"
              :                               "source-llm"
            }`}>
              {msg.source.startsWith("TOOL")
                ? <><Zap size={9} className="inline mr-1" />{msg.source.split(":")[1] ?? "Tool"}</>
                : msg.source === "RAG"
    ...
    <div className="flex flex-col items-center justify-center h-full gap-6 px-6 text-center">
      ...
      <div>
        ...
        <p className="text-sm" style={{ color: "var(--text-muted)" }}>
          Powered by RAG · real-time tools · and LLM reasoning.
        </p>
      </div>
      <div className="grid grid-cols-1 gap-2 w-full">
        {suggestions.map(q => (
          <button key={q}
            className="text-left px-4 py-2.5 rounded-xl text-xs transition-all hover:scale-[1.02]"
            style={{ background: "var(--surface2)", border: "1px solid var(--border)", color: "var(--text-muted)" }}
            onClick={() => window.dispatchEvent(new CustomEvent("suggest-prompt", { detail: q }))}>
...
export default function ChatWidget({ isAuthenticated, userId, onAuthRequired }: ChatWidgetProps) {
  ...
  useEffect(() => {
    ...
    ws.connect({
      onStatusUpdate: () => setIsTyping(true),

      onChunk: (_, fullAnswer) => {
        setIsTyping(false);
        setIsStreaming(true);
        if (!streamingMsgId.current) {
          ...
          setMessages(prev => [...prev, {
            id, type: "ai", message: fullAnswer,
            created_at: new Date().toISOString(), isNew: true, isStreaming: true,
          }]);
        } else {
          setMessages(prev => prev.map(m =>
            m.id === streamingMsgId.current ? { ...m, message: fullAnswer } : m
          ));
        }
      },
      ...
      onClose: () => { setIsTyping(false); setIsStreaming(false); },
    }).catch(() => setWsError(true));

    return () => { ws.disconnect(); chatWSRef.current = null; };
  }, [isOpen]);
  ...
  const openPanel = () => {
    ...
    setIsOpen(true); setIsClosing(false);
  };
  ...
  const handleSend = () => {
    ...
    setInput("");
    setMessages(prev => [...prev, {
      id: Date.now().toString(), type: "query",
      message: text, created_at: new Date().toISOString(), isNew: true,
    }]);
  ...
  const tabs: { id: Tab; icon: React.ReactNode; label: string; show: boolean }[] = [
    { id: "chat",      icon: <MessageSquare size={12} />, label: "Chat",      show: true },
    { id: "history",   icon: <Clock size={12} />,         label: "History",   show: true },
    { id: "knowledge", icon: <BookOpen size={12} />,      label: "Knowledge", show: RAG_UPLOAD_ENABLED },
  ];
    ...
    <>
      ...
      {(isOpen || isClosing) && (
        ...
        >
              ...
              <div className="px-4 py-4 flex-shrink-0"
                ...
                <div className="flex items-end gap-2 p-2 rounded-2xl"
                  ...
                  <textarea
                    ...
                    style={{ color: "var(--text)", fontFamily: "DM Mono, monospace", lineHeight: "1.5" }}
                  />
                  <button onClick={handleSend} disabled={!input.trim() || isBusy}
                    ...
                    style={{
                      background: input.trim() && !isBusy
                        ? "linear-gradient(135deg, var(--accent), #9c6fff)" : "var(--border)"
                    }}>
                    {isBusy
                  ...
                  <div key={i} className="p-3 rounded-xl msg-enter"
                    ...
                    <div className="flex items-center gap-2 mb-1.5">
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center ${
                        ...
                        {msg.type === "query" ? <User size={10} color="white" /> : <Bot size={10} color="white" />}
                      </div>
                      ...
                      {msg.ai_source && (
                        <span className={`text-xs px-2 py-0.5 rounded-full border ${
                          msg.ai_source.startsWith("TOOL") ? "source-tool"
                          : msg.ai_source === "RAG"        ? "source-rag"
                          :                                   "source-llm"
                        }`}>
                          {msg.ai_source.startsWith("TOOL")
                            ? msg.ai_source.split(":")[1] ?? "Tool"
                            : msg.ai_source}
                        </span>
                      )}
                    </div>
                    <p className="text-xs leading-relaxed" style={{ color: "var(--text)" }}>{msg.message}</p>
                  </div>
                ))
              )}
            </div>
          ...
          {/* ── Knowledge tab ──────────────────────────────────────────── */}
          {activeTab === "knowledge" && RAG_UPLOAD_ENABLED && (
